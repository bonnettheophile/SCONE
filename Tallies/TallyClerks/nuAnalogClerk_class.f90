module nuAnalogClerk_class
  ...
  integer(longInt), parameter  :: NU_FISS  = 0, &   ! Number of neutrons generated by fission
                                  NU2_FISS = 1, &   ! Square of number of neutrons generated by fission
                                  TMP      = 2, &   ! Temporary for computing NU**2
...
contains
  ...
  subroutine print(self, outFile, mem)
    ...
    type(scoreMemory), intent(in)    :: mem
    real(defReal)                    :: nu, nu2, events, nu_score, STD, STDnu, STDnu2, STDevents
    ...
    do i = 1, product(resArrayShape)
      ...
      ! the STD will be NaN
      if (self % batches /= 0) then
        call mem % getResult(nu2, STDnu2, addr + NU2_FISS, self % batches)
        if (self % delayed) then 
          call mem % getResult(events, STDevents, self % getMemAddress() + EVENT, self % batches)
        else
          call mem % getResult(events, STDevents, addr + EVENT, self % batches)
        ...
        call mem % getResult(nu2, STDnu2, addr + NU2_FISS)
        if (self % delayed) then 
          call mem % getResult(events, STDevents, self % getMemAddress() + EVENT)
        else
          call mem % getResult(events, STDevents, addr + EVENT)
      ...
      nu_score = nu2/events
      STD = nu_score * sqrt((STDnu2/nu2)**2 + (STDevents/events)**2)
