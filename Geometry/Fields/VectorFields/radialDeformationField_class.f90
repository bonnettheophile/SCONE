module radialDeformationField_class
  ...
  !! }
  !!
  type, public, extends(deformationField) :: radialDeformationField
...
contains
  ...
  subroutine init(self, dict)
    class(radialDeformationField), intent(inout)          :: self
    ...
    if (self % variableAmplitude) then
      origin(1) = - ONE / ((self % pitch(3) / TWO) ** 2)
      origin(2) = ZERO
      origin(3) = ONE
      call self % deformationAmplitude % init(origin)
      origin = ZERO
    end if
  ...
  function forward(self, coords, X) result(r)
    class(radialDeformationField), intent(in) :: self
...
end module radialDeformationField_class
